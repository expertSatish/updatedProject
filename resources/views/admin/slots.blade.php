            ...
            <div class="row MainBoxAc">
                <div class="col-md-9">
                    ...
                    <div class="card UserBox p-0 EditSlots ">
                        ...
                        @endphp
                        <form
                            class="card-body fltform px-sm-3 px-2 py-3 d-flex justify-content-between align-items-start">
                            @csrf
                            <div>
                                <select class="form-control form-select" name="year" onchange="$('.fltform').submit();">
                            ...
                            <div><a href="{{ route('admin.copyfornextweek', ['id' => $id ,'start' => $startMonth, 'end' => $endMonth]) }}"
                        ...
                        <div class="w-100 d-block">
                            <div class="SlotingDate text-center">
                                @for ($i = $startMonth; $i <= $endMonth; $i++) @php
                                    $checkav=\App\Models\SlotAvailability::where(['date'=>$i,'expert_id'=>$id])->get();
                                    @endphp
                                    <div
                                        class="sitem {{ date('Y-m-d') > date('Y-m-d', strtotime($i)) ? 'no-drop' : '' }}">
                                        ...
                                        <ul>
                                            ...
                                            <li>
                                                <span>{{ date('H:i', strtotime($avai->from_time)) }}-{{ date('H:i', strtotime($avai->to_time)) }}
                                                    <a href="#AddTime" data-bs-modaltype="editavailability"
                                                        ...
                                                        data-bs-to="{{ $avai->to_time }}" data-bs-id="{{ $avai->id }}"
                                            ...
                                            <li><a href="#AddTime" data-bs-date="{{ date('Y-m-d', strtotime($i)) }}"
                                                    data-bs-datetype="{{ date('D (d-M)', strtotime($i)) }}"
                                                    data-bs-type="{{ date('D', strtotime($i)) }}"
                                                    class="sws-top sws-bounce" data-bs-toggle="modal"
                                                    data-title="Add Availability"><i class="fal fa-plus-circle"></i></a>
                                            </li>
                                        </ul>
                                    </div>
                                    @endfor
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <form class="modal-content saveslottime">
            ...
            <div class="modal-body py-3 p-4">
                ...
                @php
                $price = \App\Models\SlotCharge::where(['expert_id' => $id, 'slot_time_id' => $item->id])->first();
                @endphp
...
if (initialSlide > 7 && initialSlide < 15) {
    initialSlide = 7;
}
if (initialSlide > 14 && initialSlide < 22) {
    initialSlide = 14;
}
if (initialSlide > 21 && initialSlide < 29) {
    initialSlide = 21;
}
