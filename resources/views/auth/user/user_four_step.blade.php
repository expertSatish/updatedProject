...
.searchboxdata li:first-child {
    border: none
}

.searchboxdata li:hover {
    background: rgb(var(--blackrgb)/.08)
}

.searchboxdata li:hover a {
    background: none !important
}
...
.searchboxdata li a.img div {
    width: calc(100% - 45px)
}

.searchboxdata li a span,
.searchboxdata li a small {
    display: -webkit-box;
    overflow: hidden;
    -webkit-box-orient: vertical;
    -webkit-line-clamp: 1;
                ...
                <div class="col-lg-7 col-md-8">
                    <div class="row align-items-end">
                        <div class="col-12 text-center">
                            <h2 class="mt-2 h4">User Sign Up</h2>
                            <p class="text-secondary">Create your ExpertBells account</p>
                        </div>
                    </div>
                    <hr class="border-bottom border-secondary m-0 mb-4">
                    <form action="{{route('user.fourSave')}}" method="post" class="card card-body formbox mt-3">
                        @csrf
                        <div class="row">
                            <div class="col-lg-12 mb-2">
                                <div class="CustomerInfo" id="s5" style="padding: 10px;">
                                    <h3 class="h5 thm">Please select your minimum 5 Objective</h3>
                                    <div class="ListBox LBorder mt-2 w-100">
                                        <div class="row">
                                            @foreach($expertise as $res)
                                            <div class="col-md-6">
                                                <div class="RadioBox mb-2">
                                                    <input type="checkbox" class="btn-check" name="objectives[]"
                                                        value="{{$res->id}}" id="expertise{{$res->id}}"
                                                        autocomplete="off">
                                                    <label class="btn" for="expertise{{$res->id}}">{{$res->title}}</label>
                                                </div>
                                            </div>
                                            @endforeach
                                        </div>
                                    ...
                                    <div class="col-lg-6 mb-2">
                                        <button class="btn  px-4 btn-thm2 sbtn">Submit</button>
                                    </div>
                                </div>
                            </div>
                        </div>
