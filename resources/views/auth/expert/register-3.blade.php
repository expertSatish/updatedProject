        ...
        <div class="container">
            ...
            <div class="row justify-content-center">
                <div class="col-lg-7 col-md-8">
                    ...
                    <form action="{{route('expert.saveregisteration',['step'=>'s3'])}}" method="post" class="card card-body formbox mt-3">
                        @csrf
                        <div class="row">
                            <div class="col-lg-12 mb-2">
                                <div class="">                                    
                                    <div class="CustomerInfo" id="s3">
                                        <small class="num text-secondary d-block mb-3 text-center"><span class="thm">3 <i class="fal fa-long-arrow-right"></i></span> Step</small>
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-6">
                                                    <div class="RadioBox mb-2">
                                                        <input type="radio" class="btn-check" name="working" @checked(old('working',expertinfo()->currently_working_as)==$working->id) value="{{$working->id ?? 0}}" id="curr{{$working->id ?? 0}}" autocomplete="off">
                                                        <label class="btn" for="curr{{$working->id ?? 0}}">{{$working->title ?? ''}}</label>
                                                    </div>
                                                </div>
                                                @endforeach                                                
                                            </div>
                                            @error('working') <small class="error">{{$message}}</small> @enderror                                            
                                        </div>
                                        <div class="text-center d-flex justify-content-between align-items-center mt-3">
                                            {{-- <a href="{{route('expert.register')}}" class="btn btn-sm formbtn BackToStep sws-top sws-bounce" data-title=""><i class="fal fa-arrow-left me-2"></i> Back</a> --}}
                                            <button class="btn ms-2 px-4 bg-thm checkstep3 formbtn Next sws-bottom" onclick="$('.checkstep3').hide(); $('.p-checkstep3').show();" data-title="Next"> Next <i class="fal fa-arrow-right"></i></button>
                                            <button type="button" class="btn px-4 bg-thm p-checkstep3 formbtn Next sws-bottom pbtn disabled" style="display:none;" data-title="Next"> <i class="fad fa-spinner-third fa-spin"></i> Loading... </button>
                                        </div>
                                    </div>
                                </div>
                            </div>
