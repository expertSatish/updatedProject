        ...
        <div class="container">
            ...
            <div class="row justify-content-center">
                <div class="col-lg-7 col-md-8">
                    ...
                    <form action="{{route('expert.saveregisteration',['step'=>'s5'])}}" method="post" class="card card-body formbox mt-3">
                        @csrf
                        <div class="row">
                            <div class="col-lg-12 mb-2">
                                <div class="CustomerInfo" id="s5">
                                    <small class="num text-secondary d-block mb-3 text-center"><span class="thm">5 <i class="fal fa-long-arrow-right"></i></span> Step</small>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-6">
                                                <div class="RadioBox mb-2">
                                                    <input type="checkbox" class="btn-check" name="expertise[]" @checked(in_array($expertise->id,$extr) ? true : false) value="{{$expertise->id}}" id="expertise{{$expertise->id}}" autocomplete="off">
                                                    <label class="btn" for="expertise{{$expertise->id}}">{{$expertise->title}}</label>
                                                </div>
                                            </div>
                                            @endforeach                                          
                                        </div>
                                    ...
                                    <div class="text-center d-flex justify-content-between align-items-center mt-3">
                                        <a href="{{route('expert.register',['type'=>'s4'])}}" class="btn btn-sm formbtn BackToStepO sws-top sws-bounce" data-title=""><i class="fal fa-arrow-left me-2"></i> Back</a>
                                        <button class="btn ms-2 px-4 bg-thm checkstep5 formbtn Next sws-bottom" onclick="$('.checkstep5').hide(); $('.p-checkstep5').show();" data-title="Next"> Next <i class="fal fa-arrow-right"></i></button>
                                        <button type="button" class="btn px-4 bg-thm p-checkstep5 formbtn Next sws-bottom pbtn disabled" style="display:none;" data-title="Next"> <i class="fad fa-spinner-third fa-spin"></i> Loading... </button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script type="text/javascript">
    $('input[name="expertise[]"]').on('click',function(){
        var checkboxcheck = $('input[name="expertise[]"]:checked').length;
        if(checkboxcheck > 2){
            toastr.error('Sorry! you can`t add more than 2 expertise.');
            return false;
        }
        return true;
    });
