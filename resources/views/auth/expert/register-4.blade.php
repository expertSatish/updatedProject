        ...
        <div class="container">
            ...
            <div class="row justify-content-center">
                <div class="col-lg-7 col-md-8">
                    ...
                    <form action="{{route('expert.saveregisteration',['step'=>'s4'])}}" method="post" class="card card-body formbox mt-3">
                        @csrf
                        <div class="row">
                            <div class="col-lg-12 mb-2">
                                <div class="CustomerInfo" id="s4">
                                    <small class="num text-secondary d-block mb-3 text-center"><span class="thm">4 <i class="fal fa-long-arrow-right"></i></span> Step</small>
                                    <h3 class="h5 thm">What is the name of your Company? <span class="text-danger">*</span></h3>
                                    <div class="input-group">
                                        <input type="text" class="form-control" required name="company_name" value="{{old('company_name',expertinfo()->compnay_name)}}" placeholder="amazon, flipkart">
                                        ...
                                        <div class="dropdown FilterDrop m-0">
                                            ...
                                            <div class="dropdown-menu p-3">
                                                <ul>
                                                    ...
                                                    <li class="form-check check">
                                                        <input class="form-check-input" type="radio" value="{{$role->id}}" data-name="{{$role->title}}" @checked(in_array($role->id,old('role') ?? []) ? true : false) data-bs-type="Role" name="role[]" id="role{{$role->id}}">
                                    ...
                                    <div class="input-group mt-3 otherrolebox" style="display: {{in_array(27,old('role') ?? [])=='27'?'':'none'}}">
                                        <input type="text" name="other_role" id="otherrole" class="form-control" placeholder="Write Other Role Here...">
                                    </div>
                                    @if(in_array(27,old('role')??[]) && empty(old('other_role'))) <span class="error">Other role field is required.</span> @enderror
                                    

                                    <div class="d-flex justify-content-between align-items-center mt-3">
                                        <div class="text-start">                                        
                                            <a href="{{route('expert.register',['type'=>'s3'])}}" class="btn btn-sm formbtn BackToStep sws-top sws-bounce" data-title=""><i class="fal fa-arrow-left me-2"></i> Back</a>
                                        ...
                                        <div class="text-end">
                                            <button class="btn checkstep4 formbtn Next sws-bottom" data-title="Next"><i class="fal fa-arrow-right"></i></button>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<style>
    ...
    @media only screen and (max-width:767px){.FilterDrop .dropdown-menu ul{-webkit-column-count:1;-moz-column-count:1;column-count:1;grid-column-gap:0;-webkit-column-gap:1;-moz-column-gap:1;column-gap:1;}}
    @media only screen and (max-width:575px){.FilterDrop .dropdown-menu{min-width:inherit;}}
...
@push('js')
<script>
$('.FilterDrop .dropdown-menu .form-check-input').on('change', function(e) {
    e.preventDefault();
    var exp = $(this).data('name');
    if(exp.toLowerCase()=='other'){
        $('.otherrolebox').show();
        $('#otherrole').attr('required','required')
    }else{
        $('.otherrolebox').hide();
        $('.other_role').val('');
        $('#otherrole').removeAttr('required')
    }
    $(this).closest(".FilterDrop").find('.dropdown-toggle').text(exp);
});
