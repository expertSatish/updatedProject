            ...
            <div class="row justify-content-center">
                ...
                <div class="col-lg-7 col-md-8">
                    <div class="row align-items-end">
                        <div class="col-12 text-center mt-5">
                            <h2 class="mt-2 h4">Account Exists!</h2>
                            <p class="text-secondary mb-0">This account is already registered. Do you want to resume this account?</p>
                            <form action="{{route('resumeexpertaccount')}}" method="post">
                                @csrf
                                <input type="hidden" name="warning_email" value="{{Session::get('email')}}">
                                <input type="hidden" name="warning_password" value="{{Session::get('password')}}">
                                <button class="btn btn-thm2 warsbtn mb-2" onclick="$('.warpbtn').show(); $('.warsbtn').hide(); ">Yes</button>
                                <button type="button" style="display: none;" disabled class="btn btn-thm2 mb-2 warpbtn"><i class="fad fa-spinner-third fa-spin"></i> &nbsp;</button>
                            </form>
                            <p>
                                <a href="{{route('expertregister')}}#s1" style="font-size: 12px;" class="text-secondary">Start over with a new ID</a>
                            </p>
                ...
                <div class="col-lg-7 col-md-8">
                    ...
                    <form action="{{route('saveexpertregister')}}" method="post" class="card card-body formbox mt-3">
                        @csrf
                        <input type="hidden" name="name" value="{{request('name',expertinfo()->name ?? '')}}">
                        <div class="row">
                            <div class="col-lg-12 mb-2">
                                <div class="">                                    
                                    <div class="CustomerInfo" id="s2">
                                        <small class="num text-secondary d-block mb-3 text-center"><span class="thm">2 <i class="fal fa-long-arrow-right"></i></span> Step</small>
                                        <h3 class="h5 thm">What is your Email id, Contact No. & Current Address? <span class="text-danger">*</span></h3>
                                        <div class="input-group">
                                            <input type="text" class="form-control" name="email" value="{{old('email',expertinfo()->email ?? '')}}" placeholder="name@example.com">
                                        </div>
                                        ...
                                        <div class="input-group CountryCode mt-3">
                                            ...
                                            <input type="number" class="form-control" maxlength="10" value="{{old('mobile',expertinfo()->mobile ?? '')}}" onkeypress="return isNumber(event)" name="mobile" placeholder="Enter Phone No.">
                                        ...
                                        <div class="input-group mt-3">
                                            <textarea class="form-control" name="address" placeholder="What is your current address...">{{old('address',expertinfo()->address ?? '')}}</textarea>
                                        </div>
                                        <small class="text-secondary">Max Word : 300</small>
                                        @error('address')<br> <small class="error">{{$message}}</small>@enderror
                                        <div class="text-center d-flex justify-content-between align-items-center mt-3">
                                            <a href="{{route('expertregister')}}" class="btn btn-sm formbtn BackToStep sws-top sws-bounce" data-title=""><i class="fal fa-arrow-left me-2"></i> Back</a>
                                            <button class="btn ms-2 px-4 bg-thm checkstep2 formbtn Next sws-bottom" onclick="$('.checkstep2').hide(); $('.p-checkstep2').show();" data-title="Next"> Next <i class="fal fa-arrow-right"></i></button>
                                            <button class="btn ms-2 px-4 bg-thm p-checkstep2 formbtn Next sws-bottom" style="display:none;" disabled><i class="fad fa-spinner-third fa-spin"></i> Loading...</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<style>
    ...
    .CustomerInfo textarea.form-control{height:90px;resize:none;}
