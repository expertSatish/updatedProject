                    ...
                    <div class="LeftPanelSec">
                        <div class="Leftpanel MLeftpanel">
                            ...
                            <ul id="MsgMenu" class="collapse">
                                <li class="{{Request::segment(3)==''?'active':''}}"><a href="{{route('expert.message')}}"><i class="fal fa-inbox me-2"></i><span>Inbox <span class="badge bg-info ms-3">{{$unread}}</span></span></a></li>
                                <li class="{{Request::segment(3)=='sent'?'active':''}}"><a href="{{route('expert.message',['type'=>'sent'])}}"><i class="fal fa-envelope me-2"></i> <span>Sent</span></a></li>
                                {{-- <li><a href="{{route('expert.message',['type'=>'star'])}}"><i class="fal fa-star me-2"></i> <span>Star</span></a></li> --}}
                                <li class="{{Request::segment(3)=='trash'?'active':''}}"><a href="{{route('expert.message',['type'=>'trash'])}}"><i class="fal fa-trash me-2"></i> <span>Trash</span></a></li>
                                <li class="{{Request::segment(3)=='archive'?'active':''}}"><a href="{{route('expert.message',['type'=>'archive'])}}"><i class="fal fa-archive me-2"></i> <span>Archive</span></a></li>
                            </ul>
                        </div>
                    </div>
                                ...
                                <div class="MBThead">
                                    <div class="row">
                                        ...
                                        <div class="col-sm-5 col-8 order-sm-first">
                                            ...
                                            <button type="button" onclick="$('#formbox').attr('action','{{route('expert.bulkarchivemessage')}}?previous={{url()->current()}}'); $('#formbox').attr('method','POST'); $('#formbox').submit();" class="btn btntag m-0 me-2 sws-top sws-bounce" data-title="Archive"><i class="far fa-archive"></i></button>
                                            <button type="button" onclick="$('#formbox').attr('action','{{route('expert.bulkdeletemessage')}}?previous={{url()->current()}}');  $('#formbox').attr('method','POST'); $('#formbox').submit();" class="btn btntag m-0 me-2 sws-top sws-bounce" data-title="Delete"><i class="far fa-trash-alt"></i></button>
                                        </div>
                                ...
                                <div class="MBmails">
                                    <ul>
                                        ...
                                        @foreach ($lists as $item)
                                            <li class="{{$item->send_by==2 && $item->read_to==0?'unread':''}}">
                                                ...
                                                <a href="{{route('expert.message',['type'=>base64_encode($item->id)])}}">
                                                    <div class="name">
                                                        <span>{{$item->user->name ?? '-----'}} <small>
                                                            @if(!empty($item->user->email)) ({{$item->user->email ?? ''}}) @endif
                                                        </small></span>
                                                    </div>
