    ...
    <section class="grey pt-3">
        <div class="container">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{route('user.message')}}">Message</a></li>
                ...
                <div class="col-md-9">
                    <div class="LeftPanelSec">
                        <div class="Leftpanel MLeftpanel">
                            ...
                            <ul id="MsgMenu" class="collapse">
                                <li class="active"><a href="{{route('user.message')}}"><i class="fal fa-inbox me-2"></i><span>Inbox <span class="badge bg-info ms-3">{{$unread}}</span></span></a></li>
                                <li><a href="{{route('user.message',['type'=>'sent'])}}"><i class="fal fa-envelope me-2"></i> <span>Sent</span></a></li>
                                {{-- <li><a href="{{route('user.message',['type'=>'star'])}}"><i class="fal fa-star me-2"></i> <span>Star</span></a></li> --}}
                                <li><a href="{{route('user.message',['type'=>'trash'])}}"><i class="fal fa-trash me-2"></i> <span>Trash</span></a></li>
                                <li><a href="{{route('user.message',['type'=>'archive'])}}"><i class="fal fa-archive me-2"></i> <span>Archive</span></a></li>
                            </ul>
                        </div>
                    </div>
                    <div class="MailBox card p-0">
                        <div class="MailBoxR">
                            ...
                            <input type="hidden" id="replybox" value="{{$list->expert->email ?? ''}}">
                            ...
                            <div class="MBThead">
                                <div class="row">
                                    ...
                                    <div class="col-5">
                                        @if($list->send_by==1  && count($list->reply)==0)
                                            @php 
                                        ...
                                        @if($list->user_id==userinfo()->id)
                                            @if($list->archive_to==0)
                                                <a href="{{route('user.archivemessage',['previous'=>url()->previous(),'id'=>$list->id])}}" class="btn btntag m-0 me-2 sws-top sws-bounce" data-title="Archive"><i class="fal fa-archive"></i></a>
                                            @else
                                                <a href="{{route('user.archivemessage',['previous'=>url()->previous(),'id'=>$list->id])}}" class="btn btntag m-0 me-2 sws-top sws-bounce" data-title="Un-Archive"><i class="fal fa-archive"></i></a>
                                            @endif
                                            @if($list->delete_to==0)
                                                <a href="{{route('user.deletemessage',['previous'=>url()->previous(),'id'=>$list->id])}}" class="btn btntag m-0 me-2 sws-top sws-bounce" data-title="Delete"><i class="fal fa-trash-alt"></i></a>
                                            @elseif($list->delete_to==1)
                                                <a href="{{route('user.deletemessage',['previous'=>url()->previous(),'id'=>$list->id])}}" class="btn btntag m-0 me-2 sws-top sws-bounce" data-title="Moved From Trash"><i class="far fa-arrows"></i></a>
                                                <a href="{{route('user.deletemessage',['previous'=>url()->previous(),'id'=>$list->id,'confirm'=>true])}}" class="btn btntag m-0 me-2 sws-top sws-bounce" data-title="Delete"><i class="fal fa-trash-alt"></i></a>
                                            @endif
                                        @endif 
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="MailHead">
                                        <span class="uimg NameIcon">
                                            <span>{{$list->expert->name ?? 'Dear'}}</span>
                                        </span>
                                        <div class="NameDate">
                                            <span>{{$list->expert->name ?? 'Dear'}}</span>
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="dropdown dropend">
                                                    ...
                                                    <div aria-labelledby="dd_id1" class="dropdown-menu">
                                                        <a href="#" class="dropdown-item"><span>from:</span>  {{$list->user->email ?? '------'}}</a>
                                                        <a href="#" class="dropdown-item"><span>to:</span>  {{$list->expert->email ?? '-----'}}</a>
