                                    ...
                                    <div class="col-md-12">
                                        <div class="WalletMain grey d-flex align-items-center flex-wrap justify-content-between">
                                            <div>
                                                ...
                                                <h2 class="m-0 h4">{{defaultcurrency()}} {{userinfo()->wallet}}</h2>
                                            </div>
                                            <div>
                                                <button data-bs-toggle="modal" data-bs-target="#DepositModal" class="btn btn-thm2 m-0">Deposit</button>
                                                <button data-bs-toggle="modal" data-bs-target="#WithdrawalModal" class="btn btn-thm2 m-0 ms-2">Withdraw</button>
                                                <div class="MissingD d-flex justify-content-between">Missing Deposit<a data-bs-toggle="modal" href="#ClaimModal"><strong>Claim Now ?</strong></a></div>
                                            </div>                                            
                                        </div>
                                        <small><b>NOTE:</b> You can deposit minimum <b>{{settingdata()->minimum_deposit}} {{defaultcurrency()}}</b> in wallet and  minimum withdraw amount is <b>{{settingdata()->minimum_withdraw}} {{defaultcurrency()}}</b>.</small>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="item">
                                <div class="detail">
                                    ...
                                    <div>
                                        ...
                                        @if($item->type=='refund') <p class="mb-0">refunded {{defaultcurrency()}} {{$item->amount}} for booking cancellation.</p> @endif
                                        @if($item->type=='purchase') <p class="mb-0">We have received your payment for the booking. <a href="{{route('user.scheduleinfo',['booking'=>$item->booking->booking_id ?? 0])}}" target="_blank">#{{$item->booking->booking_id ?? 0}}</a>.</p> @endif
                                        @if($item->type=='deposit') 
                                            @if($item->payment==0) <p class="mb-0"> Your deposit payment under processing....</p>  @endif
                                            @if($item->payment==1) <p class="mb-0"> Your deposit payment has been received & your wallet updated.</p>  @endif
                                            @if($item->payment==2) <p class="mb-0"> Your deposit payment has been canceled / declined.</p>  @endif
                                        @endif
                                        @if($item->type=='purchase') <p class="mb-0"></p> @endif
                                ...
                                <div class="right">
                                    @if($item->type=='refund') <div class="price text-success"> + {{defaultcurrency()}} {{$item->amount}}</div> @endif
                                    @if($item->type=='deposit') <div class="price text-{{$item->payment==0?'secondary':''}}{{$item->payment==1?'success':''}}{{$item->payment==2?'danger':''}}"> + {{defaultcurrency()}} {{$item->amount}}</div> @endif
                                    @if($item->type=='purchase') <div class="price text-danger"> - {{defaultcurrency()}} {{$item->amount}}</div> @endif
                                    @if($item->type=='withdrawal') <div class="price text-{{$item->is_publish==0?'secondary':''}}{{$item->is_publish==1?'danger':''}}{{$item->is_publish==2?'success':''}}"> - {{defaultcurrency()}} {{$item->amount}}</div> @endif    
...
@endpush
@push('js')
    <div class="modal fade" id="DepositModal" data-bs-backdrop="static" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <form method="POST" action="{{route('user.depositmoney')}}" class="modal-content depositfrm">
                @csrf
                <div class="modal-header justify-content-center">
                    <div class="text-center">
                        <h6 class="modal-title" id="exampleModalLabel">DEPOSIT MONEY</h6>
                        <small>You can deposit minimum {{settingdata()->minimum_deposit}} {{defaultcurrency()}}.</small>     
                    </div>                   
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="form-group">
                                <input type="number" class="form-control" name="amount" id="depositamount" placeholder="enter deposit amount ({{defaultcurrency()}})">
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer justify-content-center">
                    <button class="btn btn-thm2 m-0 w-100 dsbtn">Comfirm & Proceed</button>
                    <button class="btn btn-thm2 m-0 w-100 dpbtn" disabled style="display: none"><i class="fad fa-spinner-third fa-spin me-1"></i> Loading...</button>
                </div>
            </form>
        </div>
    </div>
    <div class="modal fade" id="WithdrawalModal" data-bs-backdrop="static" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <form method="POST" action=""  class="modal-content withdrawalfrm">
                @csrf      
                @php 
                    $request = \App\Models\WithdrawalRequest::where(['user_id'=>userinfo()->id,'is_publish'=>0])->count();
                @endphp      
                <div class="modal-header justify-content-center">
                    <div class="text-center">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-12 text-center">
                            <h6 class="title">Sorry! {{userinfo()->name}}</h6>
                            <small>
            ...
            <form class="modal-content claimfrm">
                ...
                <div class="modal-header justify-content-center">
                    <div class="text-center">
                        ...
                        <small>You can raise a request for problems related to deposits and withdrawals here.</small>
                    </div>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row ListBox">
                        <div class="col-lg-6">
                            <div class="RadioBox">
                                <input type="radio" class="btn-check" name="request_for" value="deposit" id="DepositRelated" autocomplete="off">
                                <label class="btn" for="DepositRelated">Deposit Related</label>
                            </div>
                            <small class="error text-danger request-er"></small>
                        </div>
                        <div class="col-lg-6">
                            <div class="RadioBox">
                                <input type="radio" class="btn-check" name="request_for" value="withdrawal" id="WithdrawalRelated" autocomplete="off">
                                <label class="btn" for="WithdrawalRelated">Withdrawal Related</label>
                            </div>                            
                        </div>
    ...
    <script>
        $('.depositfrm').on('submit',function(e){
            let Amount = $('input[name=amount]').val();
            let MinimumAmount = "{{settingdata()->minimum_deposit}}";
            ...
            pbtn.show();
            if(Amount==''){
                toastr.error('The amount field is required.');
                sbtn.show();
                pbtn.hide();
                return false;
            }
            else if(Amount.toFixed(2)<MinimumAmount){
                toastr.error('You can add at least '+MinimumAmount+' {{defaultcurrency()}}');
                sbtn.show();
                pbtn.hide();
                return false;
            }else{                
        ...
        $('.withdrawalfrm').on('submit',function(e){
            ...
            $.ajax({
                ...
                url:@json(route('user.withdrawalrequest')),
                dataType:"Json",
                method:"Post",
                cache:false,
                contentType:false,
        ...
        $('.claimfrm').on('submit',function(e){
            ...
            $.ajax({
                data:new FormData(this),
                url:@json(route('user.claimrequest')),
                dataType:"Json",
                method:"Post",
                cache:false,
                contentType:false,
