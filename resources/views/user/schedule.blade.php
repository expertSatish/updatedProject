                                        ...
                                        <div class="Tbody">
                                            ...
                                            <div class="cell">
                                                ...
                                                @elseif ($booking->status == 1 && $booking->reschedule_slot == 0)
                                                    @if ($booking->call_invitation == 1 && date('Y-m-d H:i') >= $Start && date('Y-m-d H:i') <= date('Y-m-d H:i', strtotime('+10 minutes' . $End)))
                                                        <a href="{{ route('user.videocall', ['schedule' => $booking->booking_id]) }}"
                                                            target="_blank" class="h5 text-primary"><span> <button type="button" class="btn btn-success"> <i class="fal fa-video me-1"></i> Join Now</button></span></a>
                                                    @else
                                                    <span class="SessionLink"><a href="{{ route('user.videocall', ['schedule' => $booking->booking_id]) }}"
                                                        target="_blank" class="h5 text-primary disabled"><span> <button type="button" class="btn btn-success"> <i class="fal fa-video me-1"></i> Join Now</button></span></a></span>
                                                    @endif                                                   
                                                @endif
                                            </div>
