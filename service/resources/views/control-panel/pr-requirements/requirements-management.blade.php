      ...
      <div class="right_col" role="main">
        ...
        <div class="row">
          ...
          <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
              <div class="x_title">
                <h2>Pre Requirements Management</h2>
                ...
                ?>
                <a href="#AddModal2" data-toggle="modal" class="btn btn-primary pull-right"><i class="fa fa-file-text"></i> Pre-Requirment Table Title</a>
                ...
                <a href="{{url('control-panel/new-pre-requirements/'.Request::segment(3))}}" class="btn btn-primary pull-right"><i class="fa fa-plus"></i> Add New </a>
                <div class="clearfix"></div>
              </div>
              <div class="x_content">

                <form action="{{url('control-panel/remove-pre-requirements')}}" method="post">
                  {{csrf_field()}}
                  ...
                  <table id="datatable1" class="table table-striped table-bordered bulk_action">
                    ...
                    <tr>
                      ...
                      <td>{{$Rows->title}}</td>
                      <td>{{$Rows->text}}</td>
                      <td>{!! Helper::CheckDataStatus(url('control-panel/pre-requirements-status'),$Rows->id,$Rows->status) !!}</td>

                      <td>

                        {!! Helper::EditBTN(url('control-panel/update-pre-requirements/'.$Rows->id)) !!}
      ...
      <div id="AddModal2" class="modal fade" role="dialog">
        <div class="modal-dialog modal-lg">
          ...
          <div class="modal-content">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal">&times;</button>
              <h4 class="modal-title">Pre-Requirment Table Title</h4>
            </div>

            @if($heading!=null)

            <form method="post" action="{{url('control-panel/pr-requirement-list-update')}}" enctype="multipart/form-data">
              @csrf
              <div class="modal-body" style="max-height:338px;overflow:auto;">
                <div class="row">
                  <div id="imagebox1">
                    <div class="col-md-6">
                      ...
                      <input type="hidden" name="category_id" value="{{$category_id}}">
                      <input type="text" class="form-control" name="text_1" value="@if($heading->text_1){{$heading->text_1}}@endif">
                    </div>
                    <div class="col-md-5">
                      <span>Title2</span>
                      <input type="text" class="form-control" name="text_2" value="@if($heading->text_2){{$heading->text_2}}@endif">
                    </div>
                    <div class="col-md-1">
                      <span>Action</span>
                      <a href="{{url('control-panel/pr-requirement-list-delete/'.$heading->id)}}" class="btn btn-danger pull-right"><i class="fa fa-trash"></i></a>
                    </div>
                  </div>
                </div>
              </div>
            ...
            @else
            <form method="post" action="{{url('control-panel/pr-requirement-list-add')}}" enctype="multipart/form-data">
              @csrf
              <div class="modal-body" style="max-height:338px;overflow:auto;">
                <div class="row">
                  <div id="imagebox1">
